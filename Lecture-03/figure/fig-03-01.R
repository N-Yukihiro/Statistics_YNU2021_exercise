...
mode <- data.frame(var = c("x1", "x2", "x3"),
                   value = c(mlv("normal", mean = 5, sd = 0.1),
                             mlv("beta", shape1 = 10, shape2 = 2),
                             mlv("beta", shape1 = 2, shape2 = 10)))

# データの作成, グラフの描画
set.seed(2718)
withr::with_preserve_seed(
  data.frame(x1 = rnorm(n = 100000, mean = 5, sd = 0.1),
             x2 = rbeta(n = 100000, shape1 = 10, shape2 = 2),
             x3 = rbeta(n = 100000, shape1 = 2, shape2 = 10)) %>% 
    tidyr::pivot_longer(cols = c(x1, x2, x3), names_to = "var", values_to = "value") %>%
    ggplot() +
    aes(x = value,
        y = ..density..) +
    geom_density() +
