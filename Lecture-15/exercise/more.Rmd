...
```{r}
ols1 <- lm(sales  ~ youtube, data = marketing)
ml1  <- glm(sales ~ youtube, 
            family = "gaussian", data = marketing)
ml0  <- glm(sales ~ 1, 
            family = "gaussian", data = marketing)
```
...
```{r}
cv <- qt(0.025, df = ols1$df.residual,
        lower.tail = FALSE)
cv
cv < as.numeric(t)
```
...
```{r}
r2.3 <- cor(marketing$sales, marketing$youtube)^2
```
...
bic <- multiply_by(-2,
                   LL %>% as.numeric()) %>% 
  add(multiply_by(ml1$coefficients %>% length() %>% add(1),
                  marketing %>% nrow() %>% log()))
bic
```
